...
**v0.4 has shipped.** The consent ceremony is now a checked effect, not just a runtime latch. The static checker walks `PREP`, `CONSENT`, and `CLENCH` alongside the stack and rejects `INSERT`/`EXTRACT`/`FLUSH`/`BUFSET`/`STORE` on any path that has not armed the required latch — the existing `TIGHTNESS` / `REFUSAL` / `LOCKDOWN` codes now fire at probe time when the checker can decide. Branch and loop joins use a three-valued lattice (Unarmed / Armed / Top); arming on one path of a conditional is no longer enough. `anal run --ledger <path>` writes a hash-chained binary `.sphlog` file — one fixed-size record per destructive op — and `anal audit` replays it to prove the recorded run has not been edited and that the source still matches. `anal run --hard` strips ambient filesystem authority: `INGEST` and `EVACUATE` raise `OUTSIDE` (E019) unless the program has authorised the exact target through the new `REQUEST` op, which prompts the user for consent at run time. Four new error codes — `BROKEN_CHAIN` (E016), `LEDGER_DRIFT` (E017), `LEDGER_GAP` (E018), and `OUTSIDE` (E019).

What is not in v0.4 yet: a module system, FFI, network primitives, or any form of concurrency. ANAL still has no concept of nothing.
...
- **v0.4** ✓ shipped — effect-typed consent checking, the `.sphlog` audit ledger with `anal audit`, and `--hard` mode with the `REQUEST` op. Four new error codes (E016–E019).
